...
class Direction (object):
  ...
  def __init__(self, orientations_dict=None):
    ...
    if (orientations_dict == None):
      self.orientationsDict = collections.defaultdict(float)
    else :
  ...
  def addOrientationValue(self, o, v) :
    self.orientationsDict[o] += v
    self.totalOrientations += v
...
class PhrasePairTableEntry(object) :
  ...
  def addOrientationValue(self, d,o, v):
    if d == DirectionName.LEFT_TO_RIGHT:
      self.directionLR.addOrientationValue(o,v)
    elif d == DirectionName.RIGHT_TO_LEFT:
      self.directionRL.addOrientationValue(o,v)
    else:
      raise ValueError("pass a DirectionName")
      
  def getOrientationValue(self, d, o) :
    if d == DirectionName.LEFT_TO_RIGHT:
      return self.directionLR.orientationsDict[o]
    elif d == DirectionName.RIGHT_TO_LEFT:
      return self.directionRL.orientationsDict[o]
    else:
      raise ValueError("pass a DirectionName")
...
class Extractor(object):
  ...
  def getStats(self):
    ...
    s += 'Average values: '
    s += '\n\nl->r monotone pb: ' + str(self.directionLR_phraseBased.orientationsDict[Orientation.MONOTONE]/float(self.total_extracted))
    s += '\nl->r swap pb: ' + str(self.directionLR_phraseBased.orientationsDict[Orientation.SWAP]/float(self.total_extracted))
    s += '\nl->r d_l pb: ' + str(self.directionLR_phraseBased.orientationsDict[Orientation.DISC_LEFT]/float(self.total_extracted))
    s += '\nl->r d_r pb: ' + str(self.directionLR_phraseBased.orientationsDict[Orientation.DISC_RIGHT]/float(self.total_extracted))
    s += '\n\nr->l monotone pb: ' + str(self.directionRL_phraseBased.orientationsDict[Orientation.MONOTONE]/float(self.total_extracted))
    s += '\nr->l swap pb: ' + str(self.directionRL_phraseBased.orientationsDict[Orientation.SWAP]/float(self.total_extracted))
    s += '\nr->l d_l pb: ' + str(self.directionRL_phraseBased.orientationsDict[Orientation.DISC_LEFT]/float(self.total_extracted))
    s += '\nr->l d_r pb: ' + str(self.directionRL_phraseBased.orientationsDict[Orientation.DISC_RIGHT]/float(self.total_extracted))
    
    s += '\n\n\nl->r monotone wb: ' + str(self.directionLR_wordBased.orientationsDict[Orientation.MONOTONE]/float(self.total_extracted))
    s += '\nl->r swap wb: ' + str(self.directionLR_wordBased.orientationsDict[Orientation.SWAP]/float(self.total_extracted))
    s += '\nl->r d_l wb: ' + str(self.directionLR_wordBased.orientationsDict[Orientation.DISC_LEFT]/float(self.total_extracted))
    s += '\nl->r d_r wb: ' + str(self.directionLR_wordBased.orientationsDict[Orientation.DISC_RIGHT]/float(self.total_extracted))
    s += '\n\nr->l monotone wb: ' + str(self.directionRL_wordBased.orientationsDict[Orientation.MONOTONE]/float(self.total_extracted))
    s += '\nr->l swap wb: ' + str(self.directionRL_wordBased.orientationsDict[Orientation.SWAP]/float(self.total_extracted))
    s += '\nr->l d_l wb: ' + str(self.directionRL_wordBased.orientationsDict[Orientation.DISC_LEFT]/float(self.total_extracted))
    s += '\nr->l d_r wb: ' + str(self.directionRL_wordBased.orientationsDict[Orientation.DISC_RIGHT]/float(self.total_extracted))
  ...
  # f ||| e ||| p1 p2 p3 p4 p5 p6 p7 p8
    ...
    where
     ...
     p8=p_r->l(dr|(f; e))
  """
  def writePairToFile(self, f1, pair, phrasePairTableEntry, phraseBased=True) :
  ...
  def toProbsToFile(self, phraseBased=True):
    ...
    for pair, phrasePairTableEntry in phraseTable.iteritems():

      self.writePairToFile(f1 , pair, phrasePairTableEntry, phraseBased)
...
class ReorderingCalculator(object) :
  ...
  def get_next_pairs_lr(self,i, phrase_pairs):
    a = []
    for j in range(0, len(phrase_pairs)):
